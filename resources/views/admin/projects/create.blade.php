        ...
        <div class="bg-white overflow-hidden shadow-[0_30px_60px_-15px_rgba(0,0,0,0.05)] sm:rounded-[3.5rem] border border-gray-50 relative group">
            ...
            <div class="p-12 relative z-10 text-right">
                <form action="{{ isset($project) ? route('admin.projects.update', $project) : route('admin.projects.store') }}" method="POST" enctype="multipart/form-data" class="space-y-10">
                    ...
                    <div class="space-y-4">
                        <label class="block text-sm font-black text-gray-700">معرض صور المشروع (اختياري)</label>
                        <p class="text-xs font-bold text-gray-400 mb-2">قم برفع عدة صور لعرضها في صفحة تفاصيل المشروع</p>
                        
                        @if(isset($project) && $project->images->count() > 0)
                            <div class="grid grid-cols-4 gap-4 mb-4">
                                @foreach($project->images as $image)
                                    <div class="relative group">
                                        <img src="{{ Storage::url($image->image_path) }}" class="h-24 w-full object-cover rounded-xl shadow border-2 border-gray-100">
                                    </div>
                                @endforeach
                            </div>
                        @endif
                        
                        <input type="file" name="images[]" multiple accept="image/*" class="w-full text-sm text-gray-400 font-bold file:ml-4 file:py-3 file:px-6 file:rounded-xl file:border-0 file:text-xs file:font-black file:bg-gradient-to-r file:from-indigo-500 file:to-purple-600 file:text-white hover:file:from-indigo-600 hover:file:to-purple-700 transition cursor-pointer">
                        <p class="text-[10px] font-bold text-gray-400 mt-2">يمكنك اختيار عدة صور معاً (حتى 5 ميجا لكل صورة)</p>
                        @error('images.*') <p class="text-xs font-black text-red-500 mt-2">{{ $message }}</p> @enderror
                    </div>
